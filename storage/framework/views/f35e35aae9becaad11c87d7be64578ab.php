            ...
            <div class="modal-body">
                <div class="row">
                    <form id="reservationForm" method="post">
                        <?php echo csrf_field(); ?> <!-- Este token es obligatorio para las solicitudes POST en Laravel -->
                        <div class="row">
                            <div class="col-md 6">
                                <div class="mb-3">
                                    <label><b>Usuario</b></label>
                                    <?php if(Auth::check()): ?>
                                    <input type="text" name="usuario" class="form-control" value="<?php echo e(Auth::user()->nombre_completo); ?>" readonly>
                                    <input type="hidden" name="id_usuario" value="<?php echo e(Auth::user()->id_usuario); ?>">
                                    <?php else: ?>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select name="id_servicio" class="form-select" required>
                                        ...
                                        <option value="<?php echo e($servicio->id); ?>"><?php echo e($servicio->nomServicio); ?></option>
...
<div class="modal fade" id="modal-pago" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    ...
    <script>
        $(document).ready(function() {
            $('#reservationForm').on('submit', function(e) {
                ...
                $.ajax({
                    url: "<?php echo e(route('citas.store')); ?>",
                    method: 'POST',
                    data: $(this).serialize(),
                    success: function(response) {
                        if (response.success) {
                            ...
                            var detalleUrl = "<?php echo e(url('/citas/detalle-pago')); ?>/" + response.cita_id;
                            $.ajax({
                                url: detalleUrl,
                                method: 'GET',
            ...
            if (error) {
                ...
                if (result.error) {
                    ...
                    if (confirmError) {
                        ...
                        window.location.href = "<?php echo e(route('registrar_cita')); ?>";
                    }
                } else {
                    window.location.href = "<?php echo e(route('registrar_cita')); ?>";
                }
        ...
        paypal.Buttons({
            ...
            onApprove: function(data, actions) {
                return actions.order.capture().then(function(details) {
                    ...
                    $.ajax({
                        url: '<?php echo e(route("payment.paypal")); ?>',
                        method: 'POST',
                        data: {
    ...
    <script>
        ...
        function pagarCrypto() {
            const apiKey = "<?php echo e(env('COINBASE_API')); ?>";
            const chargeData = {
                            ...
                            fetch(`https://api.commerce.coinbase.com/charges/${chargeId}`, {
                                ...
                                .then(chargeData => {
                                    if (chargeData.data.timeline[0].status === 'COMPLETED') {
                                        ...
                                        $.ajax({
                                            url: '<?php echo e(route("payment.crypto.callback")); ?>',
                                            method: 'POST',
                                            data: {
                                                _token: '<?php echo e(csrf_token()); ?>',
                                                cita_id: document.getElementById('cita_id').value,
                ...
                dateClick: function(info) {
                    ...
                    if (isAuthenticated !== 'true') {
                        Swal.fire({
                            ...
                            footer: `<a href="<?php echo e(route('login.form')); ?>">¿Ya tienes una cuenta? Inicia Sesión</a>`,
                        });
                            ...
                            success: function(response) {
                                ...
                                if (response.error) {
                                    ...
                                    Swal.fire({
                                        ...
                                        backdrop: `
                                        rgba(0,0,123,0.2)
                                        url("<?php echo e(asset('images/nyan-cat-nyan.gif')); ?>")
                                        left top
                                        no-repeat
                                    `
                                    });
...
<?php echo $__env->make('layout.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\Miguel\IdeaProjects\Shine-Go\resources\views\registrar_cita.blade.php ENDPATH**/ ?>